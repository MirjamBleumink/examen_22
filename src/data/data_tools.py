...
import tifffile  # Toegevoegd door Mirjam Bleumink, notebook 1 vraag 3.
...
class EuroSatDataset(BaseDictDataset):
    ...
    def __len__(self) -> int:
        ...
        # 2 regels code toegevoegd door Mirjam Bleumink, notebook 1 vraag 1.
        length = len(self.paths)
        return length
        # Einde toevoeging Mirjam Bleumink, notebook 1 vraag 1.
        # raise NotImplementedError
    ...
    def process_data(self) -> None:
        for path in tqdm(self.paths):
            ...
            # 2 regels code aangepast door Mirjam Bleumink, notebook 1 vraag 2.
            key: int = self.name_mapping.get(class_name)             # None
            value: np.ndarray = self.dataset.get(key, np.array([]))  # None
            # Einde wijziging Mirjam Bleumink, notebook 1 vraag 2.
...
class SiameseStreamer(GenericStreamer):
    def __init__(
        ...
        # precalculated combinations of all possible different classes combinations # noqa E261
    ...
    def random_index(
        self,
    ) -> Tuple[Tuple[List[Any], int], Tuple[List[Tuple[Any, Any]], Tuple[int, int]]]: # noqa E261
        ...
        Returns
        - equal (int): this is a random class key from which to pick similar images # noqa E261
        ...
        - other (List[Tuple]). A list of tuples, every tuple contains two indexes, # noqa E261
    ...
    def batchloop(self) -> Sequence[Tuple]:
        ...
        # 4 regels code toegevoegd door Mirjam Bleumink, notebook 1 vraag 3.
        a_same = self.dataset[equal]
        a_other_i = self.dataset[i]
        a_other_j = self.dataset[j]
        # einde toevoeging Mirjam Bleumink, opdracht 3.

        for idx in same:
            # append to the batch a tuple (img1, img2, 1)
            # use tifffile to read the image
            # cast the image to np.int32
            # 5 regels code toegevoegd door Mirjam Bleumink, notebook 1 vraag 3. # noqa E261
            x1, x2 = a_same[idx[0]], a_same[idx[1]]
            x1 = tifffile.imread(x1).astype(np.int32)   # plaatje van maken
            x2 = tifffile.imread(x2).astype(np.int32)   # cast naar np.int32
            y = 1

            batch.append((x1, x2, y))
            # einde toevoeging Mirjam Bleumink, notebook 1 vraag 3.
            self.index += 1
        ...
        for idx in other:
            ...
            # cast the image to np.int32
            # 5 regels code toegevoegd door Mirjam Bleumink, notebook 1 vraag 3.  # noqa E261
            x1, x2 = a_other_i[idx[0]], a_other_j[idx[1]]
